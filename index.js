...
❏ *rere-bot* ❏`
    ...
    case 'pornhub': 
			if (isMedia && !mek.message.imageMessage ||isQuotedImage) {
				...
				owgi = await fxc7.downloadMediaMessage(boij)
				messages = await uploadTele(owgi)
				teks = tex.split("/")[0]
				teks2 = tex.split("/")[1]
				try { 
				buff = await getBuffer(`https://fxc7-api.herokuapp.com/api/maker/phub?apikey=Fxc7&url=${messages}&username=${teks}&text=${teks2}`, {method: 'get'})
				reply(mess.wait)
				sendImage(buff, mess.sukses)
			} catch(e) {
				reply(mess.error.bug)
				console.log(e)
			}
				} else {
				  reply(`\`\`\`Reply image dengan caption ${prefix + command} teks/teks2\`\`\``)
				}
		break
    case 'bucin':
